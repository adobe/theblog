...
async function translateTable(pages, index, batchSize) {
  ...
  if (batchSize<INDEX_BATCH_SIZE) index.done=true;
}
...
export async function fetchArticleIndex(offset) {
  ...
  if (isLocalhost()) {
    ...
    indexUrl=`/${window.blog.language}/query-index.json?limit=${INDEX_BATCH_SIZE}&offset=${offset}`;
  }
  ...
  if (response.ok) { 
    ...
    await translateTable(data,window.blog.articleIndex, batchSize);
  }
  console.log(`fetched article index: at ${index.articles.length} / ${offset+batchSize} entries, ${index.done?'':'not'} done.`)
}
...
async function fetchHits(filters, limit, cursor) {
  ...
  if (!window.blog.articleIndex) {
    await fetchArticleIndex(offset);
  }
